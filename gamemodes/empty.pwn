...
{
	SendClientMessage(playerid, 0xd90f00FF, "HATA | Bu komuta eriþim izniniz bulunmuyor.");
	return true;
}
...
stock HazirKayitGirisMenusu(playerid, dialog_, string[] = "")
{
	...
	switch(dialog_)
	{
		case REGISTER_METHOD: 
		{
			...
			format(datastring, sizeof(datastring), "%s{FFFFFF}Merhaba {1AFF1A}%s{FFFFFF}, Turkish Texas'a hoþ geldiniz!\n\nBu hesap kayýtlý deðil, oynamak için bir þifre oluþturun.", datastring, GetPlayerNameEx(playerid));
			Dialog_Show(playerid, KAYIT_MENUSU, DIALOG_STYLE_INPUT, "{1EC684}>{13DE8E}>{06FE9C}>{FFFFFF} Kayýt Ekraný", datastring, "Onayla", "Iptal");
		}
		case LOGIN_METHOD: 
		{
			if(strlen(playerData[playerid][pPassword]) < 1)
			{
				format(datastring, sizeof(datastring), "%s{FFFFFF}Merhaba {1AFF1A}%s{FFFFFF}, Turkish Texas'a hoþ geldin!\n\nBu hesap kayýtlý, hesapta oynamak için hemen giriþ yapabilirsin:", datastring, GetPlayerNameEx(playerid));
				Dialog_Show(playerid, GIRIS_MENUSU, DIALOG_STYLE_PASSWORD, "{1EC684}>{13DE8E}>{06FE9C}>{FFFFFF} Giriþ Ekraný", datastring, "Onayla", "Iptal");
			}
			else
			{
...
stock ShowServerRules(playerid)
{
	...
	else if(playerData[playerid][pRuleReading] <= 0)
	{
		...
		if(playerData[playerid][pRuleReading] == 0)
		{	
			format(laststring, sizeof(laststring), "\n{DE0D0D}** Kayýt iþlemine devam edebilirsiniz.");
		}
		else if(playerData[playerid][pRuleReading] == -1)
		{
			format(laststring, sizeof(laststring), "\n{7C7C7C}** Kurallarý baþtan sona okudunuz ve tümünü kabul ettiniz.");
		}
	}
	format(dialogstring, sizeof(dialogstring), "%s\n{FFFFFF}1.) Üstünlük saðlayan modlar kullanmak YASAKTIR. (Hileler, üçüncü parti yazýlým ile eklenti eklemek vb.).\n", dialogstring);
	format(dialogstring, sizeof(dialogstring), "%s2.) Herhangi bir ceza aldýðýnýzda, ceza süresini beklemeden yeni hesap açmak YASAKTIR.\n", dialogstring);
	format(dialogstring, sizeof(dialogstring), "%s3.) Herkese açýk kanallarda hakaret etmek ve arka arkaya hýzlý bir þekilde yazmak YASAKTIR.\n", dialogstring);
	if(strlen(laststring) > 0)
	{
		format(dialogstring, sizeof(dialogstring), "%s%s", dialogstring, laststring);
	}
	format(header_, sizeof(header_), "{8044A5}>{902ACE}>{BA47FF}>{FFFFFF} Zorunlu Tutulan Sunucu Kurallarý %s", (playerData[playerid][pRuleReading] <= 0) ? ("{6CFE44}[OKUNDU]") : ("{9D9D9D}[OKUNMADI]"));
...
{
	if(strlen(inputtext) < 6) return HazirKayitGirisMenusu(playerid, LOGIN_METHOD, "Geçersiz þifre!");
	...
	if(cache_num_rows())
	{
		...
		HazirKayitGirisMenusu(playerid, LOGIN_METHOD, "Yanlýþ þifre!");
	}
	...
	if(playerData[playerid][pRuleReading] < 1)
	{
		...
		if(playerData[playerid][pRuleReading] <= 0)
		{
			if(SearchDatabaseByName(GetPlayerNameEx(playerid)) == 0)
			{
				...
				Dialog_Show(playerid, EMPTY_DIALOG, DIALOG_STYLE_MSGBOX, "{BB3434}>{E11F1F}>{FF5C5C}>{FFFFFF} Bilgilendirme Kutucuðu", "• Hesap þifreniz otomatik olarak getirildi, GÝRÝÞ YAP butonuna basabilirsiniz.\n\n{7C7C7C}** Kurallara uygun þekilde davranmayý unutmayýn...", "{FF0000}OK", "");
...
Dialog:KAYIT_MENUSU(playerid, response, listitem, inputtext[])
{
	...
	if(strlen(inputtext) > 32) return HazirKayitGirisMenusu(playerid, REGISTER_METHOD, "Girdiðiniz þifre 32 karakterin üzerinde olamaz.");
	...
	if(playerData[playerid][pRuleReading] >= READING_RULE_TIME){
		...
		PlayerTextDrawShow(playerid, KayitGiris[7][playerid]);
		PlayerTextDrawSetString(playerid, KayitGiris[7][playerid], "GIRIS YAP");
		Dialog_Show(playerid, EMPTY_DIALOG, DIALOG_STYLE_MSGBOX, "{45A93C}>{34DE25}>{61FC53}>{FFFFFF} Bilgilendirme Kutucuðu", "{FF0000}•{FFFFFF} Harika, kayýt iþlemine devam edebilmek için bir adým kaldý! Þimdi {29C4D6}OYUN KURALLARI{FFFFFF} seçeneðine týklayýn...", "OK", "");
		return true;
	}
	if(playerData[playerid][pRuleReading] <= 0)
	{
...
public OnPlayerClickPlayerTextDraw(playerid, PlayerText:playertextid)
{
    ...
    else if(playertextid == KayitGiris[8][playerid])
    {
    	...
    	{
    		Dialog_Show(playerid, EMPTY_DIALOG, DIALOG_STYLE_MSGBOX, "{BB3434}>{E11F1F}>{FF5C5C}>{FFFFFF} Bilgilendirme Kutucuðu", "{FFFFFF}• Öncelik olarak bir þifreye ihtiyacýnýz var. {CDCDCD}KAYIT OL{FFFFFF} butonuna týklayýn ve þifrenizi tanýmlayýn.", "{FF0000}OK", "");
    	}
    }
    else if(playertextid == KayitGiris[12][playerid])
    {
    	Dialog_Show(playerid, EMPTY_DIALOG, DIALOG_STYLE_MSGBOX, "{008710}>{00BA16}>{00FA1D}>{FFFFFF} Hesap Sizin Mi?", "{FFFFFF}\
    		Bu sizin hesabýnýz deðilse, lütfen SA-MP baþlatýcýsýnda takma\nadýnýzý deðiþtirin ve sunucuya yeniden girin.\n\n\
    		Henüz baþkasý tarafýndan kaydedilmemiþ bir nick bulmaya çalýþýn.\
    	", "OK", "");
    }
    return true;
}
